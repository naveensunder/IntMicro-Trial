...
Overview as styled rows (no table). Bold dividers between questions.
"""
...
if st.session_state.get("preview_mode"):
    banner("👁 Instructor preview mode — viewing as student.", "info")

# ── Question overview (styled rows, not a table) ───────────────────────────────
...
if fresh["all_done"]:
    st.markdown(
        ...
        f'Your answers are locked. Return here anytime to review solutions.</div>'
        f'</div>',
        unsafe_allow_html=True
    )
