...
with tabs[1]:
    ...
    for cfg in sorted(hw_configs, key=lambda x: x.get("HW_ID","")):
        ...
        with st.expander(f"**{title}** ({hw_id})"):
    ...
    with st.expander("Add assignment"):
        n_id    = st.text_input("Homework ID (e.g. HW_WEEK3)", key="n_id")
        n_title = st.text_input("Title", key="n_title")
        n_en    = st.toggle("Enable immediately", key="n_en")
        n_dl    = st.date_input("Deadline date", key="n_dl")
        n_tm    = st.time_input("Deadline time", key="n_tm")
        n_gr    = st.number_input("Grace minutes", value=15, key="n_gr")
        n_mx    = st.number_input("Max marks", value=18, key="n_mx")
        n_in    = st.text_area("Instructions", key="n_in", height=60)
        if st.button("Add", key="add_hw"):
            if n_id.strip() and n_title.strip():
                dl_s = f"{n_dl.strftime('%Y-%m-%d')} {n_tm.strftime('%H:%M')}"
                if add_hw_config(n_id.strip(), n_title.strip(),
                                  str(n_en).upper(), dl_s, n_gr, "", n_mx, n_in.strip()):
