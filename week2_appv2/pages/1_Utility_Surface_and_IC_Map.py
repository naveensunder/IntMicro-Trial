...
C = dict(bg="white", ink="#1a1a2a", accent="#3d5a80", faint="#a0a8b8", rule="#cccccc")
...
for _a in [round(x,1) for x in np.arange(0.1,1.0,0.1)]:
    _b = round(1.0-_a,10)
    _key = f"Cobb-Douglas  (a={_a:.1f}, b={_b:.1f})"
    REGISTRY[_key] = dict(
...
REGISTRY["Quasi-linear  U = sqrt(X) + Y"] = dict(
...
def _trim_monotone(X, Y):
    """Keep only the strictly-decreasing portion — prevents CD curves turning upward."""
    ...
    for i in range(len(Y)):
        ...
        if np.isfinite(v) and v >= 0:
            if v <= prev:
                ...
                break   # curve started rising — stop
    return out
...
# ── UI: controls on top row, full-width chart below ────────────────────────────
...
with col2:
    azimuth = st.slider("Rotate 3-D surface", -180, 180, -60, step=5)

entry = REGISTRY[func_sel]

fig = plt.figure(figsize=(16.0, 7.5))
fig.patch.set_facecolor("white")
ax1 = fig.add_axes([0.04, 0.10, 0.40, 0.82])
draw_2d(ax1, entry)
