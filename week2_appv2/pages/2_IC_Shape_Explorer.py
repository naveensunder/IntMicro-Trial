...
def _trim_cd(X, Yv):
    """Keep only the strictly-decreasing part to prevent upward-turning CD tails."""
...
def _draw_cd(ax,a,levels,am,hi=None):
    ...
    for i,(k,col,a_,l_) in enumerate(zip(levels,cs,al,lw)):
        Yv=_trim_cd(X, cd_Y(X,k,a))
        mask=np.isfinite(Yv)&(Yv>=0)&(Yv<=am*1.02)
        if mask.any():
...
default_A = next((i for i,k in enumerate(KEYS) if "a=0.5" in k), 4)
default_B = next((i for i,k in enumerate(KEYS) if "Complements 1:1" in k), 0)

c1,c2 = st.columns(2)
with c1:
    sel_A = st.selectbox("Left panel", KEYS, index=default_A, key="A")
with c2:
